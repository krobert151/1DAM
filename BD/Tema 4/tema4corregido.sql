...
/*Seleccionar el nombre de aquellos departamentos en los que trabaja un 
empleado que fue contratado a lo largo del año 1993.*/
SELECT DISTINCT /*Esto se hace para que no se repita department_name*/department_name
FROM employees JOIN departments USING(department_id)
WHERE hire_date::text ILIKE '1993%';
...
FROM employees a1 JOIN employees a2 ON (a1.manager_id = a2.employee_id)
WHERE a2.last_name ILIKE ('D%') OR a2.last_name ILIKE ('H%') OR a2.last_name ILIKE ('S%');

/*Otra forma*/

SELECT a1.first_name, a1.last_name
FROM employees a1 JOIN employees a2 ON (a1.manager_id = a2.employee_id)
WHERE UPPER (LEFT(a2.last_name,1)) IN ('D','H','S');

/*Seleccionar el número de familiares (dependents) que son hijos de algún miembro de los departamentos 
de Marketing, Administration e IT.*/
